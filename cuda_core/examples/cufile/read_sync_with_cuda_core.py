...
def main():
    ...
    # Step 5: Create Buffer object from cuda.core.experimental
    ...
    # Step 8: Read file into GPU buffer using Buffer object (no need for buf_ptr_int!)
    bytes_read = file_handle.read(
        buffer=gpu_buffer,  # Pass Buffer object directly
        size=size,
        file_offset=0,
        buf_offset=0
    )
    ...
    # Step 9: Copy back to CPU and verify (like KvikIO xp.testing.assert_array_equal)
